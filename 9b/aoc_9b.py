...
def build_all_possible_rectangles(lines: [str]):
  ...
  for pair in combinations(red_tiles, 2):
    ...
    print("Seaarching for red tiles within %s using min_x: %d, max_x: %d, min_y: %d, max_y: %d" % (str(pair), min_x, max_x, min_y, max_y))
    is_colored = True
    for red_tile in red_tiles:
      if min_x < red_tile[0] < max_x and min_y < red_tile[1] < max_y:
        is_colored = False
        break
    if is_colored:
      print("Pair: %s, Area: %d" % (str(pair), area(pair[0], pair[1])))
...
class SmallestCircuitFinder(unittest.TestCase):
  ...
  def test_sample_case(self):
    ...
    self.assertEqual(find_largest_rectangle(sample_floor), 24)
